...
func maximumDetonation(A [][]int) int {
	n := len(A)
	var graph [][]int
	...
	for i := 0; i < n; i++ {
		...
		for j := 0; j < n; j++ {
			...
			if math.Pow(float64(A[i][2]), float64(2)) >= math.Pow(float64(A[i][0]-A[j][0]), 2)+math.Pow(float64(A[i][1]-A[j][1]), 2) {
...
func DFS(v int, graph [][]int, seen *map[int]bool) {
