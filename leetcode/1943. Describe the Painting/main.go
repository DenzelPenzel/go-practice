...
These segments can be represented by the 2D array painting where painting[j] = [leftj, rightj, mixj]
...
func splitPainting(segments [][]int) [][]int64 {
	...
	for _, segment := range segments {
		vals = append(vals, []int{segment[0], segment[2]})
		vals = append(vals, []int{segment[1], -segment[2]})
	}

	sort.Slice(vals, func(i, j int) bool {
		return vals[i][0] < vals[j][0] || (vals[i][0] == vals[j][0] && vals[i][1] < vals[j][1])
	})
	...
	for _, val := range vals {
		if prefix > 0 && prev < val[0] {
			ans = append(ans, []int64{int64(prev), int64(val[0]), int64(prefix)})
		}
		prefix += val[1]
		prev = val[0]
	}
