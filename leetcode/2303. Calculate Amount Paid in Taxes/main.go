...
package _303__Calculate_Amount_Paid_in_Taxes
...
func calculateTax(brackets [][]int, income int) float64 {
	...
	for i := 0; i < n; i++ {
		upper := float64(brackets[i][0])
		percent := float64(brackets[i][1])

		if upper > float64(income) {
			tax += (float64(income) - prev) * 0.01 * percent
			return tax
		}
		tax += (upper - prev) * 0.01 * percent
		prev = upper
	}
